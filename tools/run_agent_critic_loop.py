...
from tools.agent_critic.review_loop import (  # noqa: E402
    normalize_paths,
    read_task_text,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a multi-round executor + Gemini critic loop against the current repository."
    )
    ...
    parser.add_argument("--include-file", action="append", default=[], help="file included in every critic round")
    parser.add_argument(
        "--git-diff-mode",
        choices=["none", "worktree", "staged", "base"],
        default="worktree",
        help="which git diff each critic round should include",
    )
    ...
    parser.add_argument("--cwd", default=".", help="working directory for executor and critic")
    parser.add_argument("--output-dir", default=None, help="output directory for loop artifacts")
    parser.add_argument(
        "--critic-backend",
        choices=["auto", "api", "cli"],
        default="auto",
        help="critic backend; auto prefers API when GEMINI_API_KEY is available",
    )
    ...
    parser.add_argument("--critic-timeout-sec", type=int, default=900)
    ...
    parser.add_argument(
        ...
        default="accept",
        help="when to stop after a critic round",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="continue to the critic step even if the executor command exits non-zero",
    )
    parser.add_argument(
        "--critic-mock-response-file",
        action="append",
        default=[],
...
def main() -> None:
    ...
    mock_files = normalize_paths(list(args.critic_mock_response_file), cwd=cwd)
    ...
    summary = run_agent_critic_loop(
        ...
        critic_timeout_sec=int(args.critic_timeout_sec),
        critic_mock_response_files=mock_files,
    )

    print(f"[agent-critic-loop] output: {output_dir}")
    print(
        "[agent-critic-loop] "
